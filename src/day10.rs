...
fn connected_tiles(grid: &Grid<Tile>, tile: Coord) -> Vec<Coord> {
    connected_directions(*grid.get(tile).unwrap())
        .iter()
...
#[aoc_generator(day10)]
fn parse_input(input: &str) -> (Grid<Tile>, Coord) {
    const WIDTH: u32 = 140;
    ...
    let (start, _) = grid
        ...
        .unwrap();
    *grid.get_mut(start).unwrap() = Tile::SouthWest;

    (grid, start)
}

#[aoc(day10, part1)]
fn part1((grid, start): &(Grid<Tile>, Coord)) -> i32 {
...
#[aoc(day10, part2)]
fn part2(_input: &(Grid<Tile>, Coord)) -> i32 {
    0
}
